...
/*   Updated: 2023/08/28 13:38:00 by rjobert          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_graph(char **argv, t_frame *frame)
{
	...
	if (!(frame->image))
		return ;
	frame->addr = mlx_get_data_addr(frame->image, &frame->bits_per_pxl, 
			&frame->line_len, &frame->endian);
	if (argv[2])
		frame->color_setup = ft_atoi(argv[2]);
	else
		frame->color_setup = 4;
	frame->frame_init = 1;
	return ;
...
void	args_exit(void)
{
	...
	ft_putstr_fd("\targument 1 : 'Mandelbrot','Julia' or 'BurningShip' \n", 
		STDOUT_FILENO);
	ft_putstr_fd("\targument 2 : [1 to 4] for colors, otherwise default red\n", 
		STDOUT_FILENO);
	...
	ft_putstr_fd("\t./fractol Julia 3 4\n\n", STDOUT_FILENO);
}
